...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/testproject_gg_1.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final testprojectGg1 = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'testproject_gg_1.dart',
      );

      expect(
        testprojectGg1,
        isNotNull,
        reason: 'Item for executing bin/testproject_gg_1.dart not found',
      );

      expect(testprojectGg1['name'], 'testproject_gg_1.dart');
      expect(testprojectGg1['type'], 'dart');
      expect(testprojectGg1['request'], 'launch');
